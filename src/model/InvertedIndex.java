...
public class InvertedIndex {
    ...
    public ArrayList<Posting> search(String kunci) {
        String[] splitter = kunci.split(" ");
        ArrayList<Posting> result = new ArrayList<>();
        for (int i = 0; i < splitter.length; i++) {
            String string = splitter[i];
            if (i == 0) {
                result = searchOneWord(splitter[i]);
            } else {
                ArrayList<Posting> result2 = searchOneWord(splitter[i]);
